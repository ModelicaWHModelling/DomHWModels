  ...
  package Examples
    ...
    equation
      ...
      connect(nonCon.port_b1, temperature.port) annotation (Line(
          ...
          color={0,127,255},
          smooth=Smooth.None));
      connect(airFlo.ports[1], nonCon.port_a2) annotation (Line(
          points={{74,-106},{142,-106},{142,46},{120,46}},
          color={0,127,255},
          smooth=Smooth.None));
      connect(con.port_a2, mediaConversion.buiOut) annotation (Line(
          points={{42,46},{58,46},{58,16},{74,16}},
          color={0,127,255},
          smooth=Smooth.None));
      connect(mediaConversion.theIn, nonCon.port_b2) annotation (Line(
          points={{54,16},{68,16},{68,46},{82,46}},
          color={0,127,255},
          smooth=Smooth.None));
...
<li><i>31 Jan 2005</i>
 ...
 Combustion Chamber model restructured using inheritance.
  ...
  model MediaConversion
    ...
    Modelica.SIunits.Temperature buiT;
  ...
  equation
    ...
    theT = ThermoFlueGas.temperature(ThermoFlueGas.setState_phX(
        theIn.p,
        theIn.h_outflow,
        ThermoFlueGas.X_default));

    theT = buiT;

    buiOut.h_outflow = ThermoPower.Media.FlueGas.specificEnthalpy(ThermoPower.Media.FlueGas.setState_pTX(
        buiOut.p,
        buiT,
        buiOut.Xi_outflow));

     theIn.Xi_outflow[3] = inStream(buiOut.Xi_outflow[1]);
     buiOut.Xi_outflow[1] = inStream(theIn.Xi_outflow[3]);

      theIn.Xi_outflow[1] + theIn.Xi_outflow[2] + theIn.Xi_outflow[4] + theIn.Xi_outflow[5] = 1 - theIn.Xi_outflow[3];

    theIn.C_outflow = inStream(buiOut.C_outflow);
    buiOut.C_outflow = inStream(theIn.C_outflow);
