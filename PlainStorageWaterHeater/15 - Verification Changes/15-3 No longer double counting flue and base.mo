...
First implementation.
    ...
    equation
      for i in 2:(nSeg - 1) loop
        buoHeaTra[i] =  dHBU[i] + c_p*mDotD[i - 1]*(TWat[i - 1] - TWat[i]) + c_p*m_flow_C*(TWat[i - 1] - TWat[i]) - c_p*m_flow_C*(TWat[i] - TWat[i + 1])-QFlu[i];
      end for;
      ...
      buoHeaTra[1] =  dHBU[1] - c_p*m_flow_C*(TWat[1] - TWat[2]) - QFlu[1];
