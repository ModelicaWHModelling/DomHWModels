...
package WaterHeaterLib
  ...
  protected
    ...
    Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow prescribedHeatFlow[
      nSeg](each alpha=1e6) annotation (Placement(transformation(
          extent={{10,-10},{-10,10}},
          rotation=0,
          origin={82,-58})));
    Modelica.Blocks.Sources.RealExpression Q_flow_total[nSeg](y=Q_flow_tot)
      annotation (Placement(transformation(extent={{196,-74},{142,-42}})));
  public
    ...
    Modelica.SIunits.MassFlowRate m_flow_D[nSeg]( each start=0)
  ...
  public
    ...
    Modelica.Blocks.Sources.RealExpression heaCoe[nSeg](each y=2)
  ...
  //   Q_flow_WB := heatFlowSensor.Q_flow;
  //   // determine temperature difference between wall and water, see Eqn 2. page 8 of water heater manual
     T_HB = buoHeaTra.THotBas;
  //   // determine mass flow rate induced by based heat flux, see Eqn 3. page 8 of water heater manual
     m_flow_B = buoHeaTra.mFloBas;

    for i in 1:nSeg loop
      m_flow_F[i] = buoHeaTra.mFloFlu[i];
      T_H[i] = buoHeaTra.THotFlu[i];
    end for;

    //  mass balance of zone i, see Eqn 9. page 8 of water heater manual
    for i in 1:(nSeg - 1) loop
      m_flow_D[i] = buoHeaTra.mFloIn[i] + m_flow_D[i + 1] - m_flow_F[i] + m_flow_in[i]
         - m_flow_out[i];
    end for;

    //  mass balance of zone nSge, based on Eqn 9. page 8 of water heater manual
    m_flow_D[nSeg] = buoHeaTra.mFloIn[nSeg] - m_flow_F[nSeg] + m_flow_in[nSeg] -
      m_flow_out[nSeg];

    deltaH_flow_BU[1] = m_flow_B*c_p*(T_HB - vol[1].heatPort.T) * Buildings.Utilities.Math.Functions.smoothHeaviside(T_HB - vol[1].heatPort.T,smoDel);

      for j in 1:nSeg loop
          deltaH_flow_F[j] = m_flow_F[j]*c_p*(T_H[j] - vol[1].heatPort.T) * Buildings.Utilities.Math.Functions.smoothHeaviside(T_H[j] - vol[1].heatPort.T,smoDel);
      end for;
      deltaH_flow_F[1] = sum(deltaH_flow_F[1:nSeg]);
      deltaH_flow_BU[1] = deltaH_flow_BU[1] + deltaH_flow_F[1];

    // heat enthalpy of zone i, see Eqn 11. page 8 of water heater manual
    for i in 2:(nSeg - 1) loop
        deltaH_flow_BU[i] = m_flow_B*c_p*(T_HB - vol[i].heatPort.T) * Buildings.Utilities.Math.Functions.smoothHeaviside(T_HB - vol[i].heatPort.T,smoDel) * Buildings.Utilities.Math.Functions.smoothHeaviside(vol[i-1].heatPort.T - T_HB,smoDel);

      for j in 1:i loop
          deltaH_flow_F[j] = m_flow_F[j]*c_p*(T_H[j] - vol[i].heatPort.T) * Buildings.Utilities.Math.Functions.smoothHeaviside(T_H[j] - vol[i].heatPort.T,smoDel) * Buildings.Utilities.Math.Functions.smoothHeaviside(vol[i-1].heatPort.T - T_H[j],smoDel);
      end for;
      deltaH_flow_F[i] = sum(deltaH_flow_F[1:i]);
      deltaH_flow_BU[i] = deltaH_flow_BU[i] + deltaH_flow_F[i];
    end for;

    // heat enthalpy of zone nSeg, see Eqn 11. page 8 of water heater manual
      deltaH_flow_BU[nSeg] = m_flow_B*c_p*(T_HB - vol[nSeg].heatPort.T) * Buildings.Utilities.Math.Functions.smoothHeaviside(vol[nSeg-1].heatPort.T - T_HB,smoDel);

    for j in 1:nSeg loop
        deltaH_flow_F[j] = m_flow_F[j]*c_p*(T_H[j] - vol[nSeg].heatPort.T) * Buildings.Utilities.Math.Functions.smoothHeaviside(vol[nSeg-1].heatPort.T - T_H[j],smoDel);
    end for;
    deltaH_flow_F[nSeg] = sum(deltaH_flow_F[1:nSeg]);
    deltaH_flow_BU[nSeg] = deltaH_flow_BU[nSeg] + deltaH_flow_F[nSeg];
    ...
    Q_flow_tot[1] = deltaH_flow_BU[1] + c_p*m_flow_D[2]*(vol[2].heatPort.T -
      vol[1].heatPort.T) + c_p*m_flow_C[2]*(vol[2].heatPort.T - vol[1].heatPort.T);
    ...
    Q_flow_tot[nSeg] = deltaH_flow_BU[nSeg] - c_p*m_flow_C[nSeg]*(vol[nSeg].heatPort.T
       - vol[nSeg - 1].heatPort.T);
    // fixme -- see whether we could change implementation so that we do not hardcoded convConBou
    for i in (1:nSeg) loop
      //T_F[i] :=(T_F0.y - T_WL[i].T)*exp(-((h_cv_F*Modelica.Constants.pi*DFlu/T_F0.C_dot_G)
      //*(i*LFlu/nSeg))) + T_WL[i].T;
...
First implementation.
  ...
  package Examples
    ...
    model WaterHeater "Test model for water heater"
      ...
      Modelica.Blocks.Sources.Constant m_flow_in[nSeg](each k=0)
        annotation (Placement(transformation(extent={{-130,108},{-110,128}})));
      Modelica.Blocks.Sources.Constant m_flow_out[nSeg](each k=0)
    ...
    equation
      ...
      connect(m_flow_out.y, tan.m_flow_out) annotation (Line(
          points={{119,118},{74,118},{74,136.429},{29.9509,136.429}},
          color={0,0,127},
          smooth=Smooth.None));
      ...
      connect(tan.heaPor[2], T_sensor.port) annotation (Line(
          points={{28.3377,85.5073},{46,85.5073},{46,-11},{62,-11}},
          color={191,0,0},
          smooth=Smooth.None));
...
First implementation.
    ...
    equation
      ...
      connect(WatT.T, buoMasIn.TWat) annotation (Line(
          ...
          color={0,0,127},
          smooth=Smooth.None));
      connect(buoMasIn.mFloIn, mFloIn) annotation (Line(
          points={{11,14},{110,14}},
          color={0,0,127},
          smooth=Smooth.None));
      ...
      connect(buoBas.t_h, THotBas) annotation (Line(
          points={{-37,60},{52,60},{52,54},{110,54}},
          color={0,0,127},
          smooth=Smooth.None));
      ...
      connect(buoFlu.t_h, THotFlu) annotation (Line(
          points={{-41,-42},{76,-42},{76,-48},{110,-48}},
          color={0,0,127},
          smooth=Smooth.None));
      annotation (Diagram(graphics));
    end buoHeaTra;
    ...
    algorithm
      ...
      for i in 2:(nSeg-1) loop
          m_flow_BU[i] :=  mFloOutBas * Buildings.Utilities.Math.Functions.smoothHeaviside(THotOutBas - TWat[i],smoDel) * Buildings.Utilities.Math.Functions.smoothHeaviside(TWat[i-1] - THotOutBas,smoDel);

         for j in 1:nSeg loop
             mFloInFluSeg[j] :=  mFloOutFlu[j] * Buildings.Utilities.Math.Functions.smoothHeaviside(THotOutFlu[j] - TWat[i],smoDel) * Buildings.Utilities.Math.Functions.smoothHeaviside(TWat[i-1] - THotOutFlu[j],smoDel);
         end for;

         mFloInFlu[i] :=  sum(mFloInFluSeg[1:nSeg]);
      ...
      //  total buoyancy flow from lower zones which mixes into zone nSeg, see Eqn 7. page 8 of water heater manual
         m_flow_BU[1] :=mFloOutBas * Buildings.Utilities.Math.Functions.smoothHeaviside(THotOutBas - TWat[1],smoDel);

        for j in 1:nSeg loop
            mFloInFluSeg[j] :=  mFloOutFlu[j] * Buildings.Utilities.Math.Functions.smoothHeaviside(THotOutFlu[j] - TWat[1],smoDel);
        end for;
      ...
      for j in 1:nSeg loop
          mFloInFluSeg[j] := mFloOutFlu[j] * Buildings.Utilities.Math.Functions.smoothHeaviside(TWat[nSeg - 1] - THotOutFlu[j],smoDel);
      end for;

      mFloInFlu[nSeg] := sum(mFloInFluSeg[1:nSeg]);
...
First implementation.
    ...
    package Examples
      ...
      model buoHeaTraExample
        ...
        buoHeaTra buoHeaTra1(nSeg=3, redeclare package Medium = Medium)
          annotation (Placement(transformation(extent={{42,4},{62,24}})));
        Modelica.Blocks.Sources.RealExpression realExpression1(y=30)
          annotation (Placement(transformation(extent={{-90,64},{-70,84}})));
        Modelica.Blocks.Sources.RealExpression realExpression3[3](y=60)
          annotation (Placement(transformation(extent={{-88,-56},{-68,-36}})));
        Modelica.Blocks.Sources.Constant const[3](k=273.15 + 65)
          annotation (Placement(transformation(extent={{-94,-16},{-74,4}})));
        Modelica.Thermal.HeatTransfer.Sources.PrescribedTemperature prescribedTemperature[3]
          annotation (Placement(transformation(extent={{-36,-16},{-16,4}})));
        Modelica.Thermal.HeatTransfer.Sources.PrescribedTemperature
          prescribedTemperature1
          annotation (Placement(transformation(extent={{-36,26},{-16,46}})));
        Modelica.Blocks.Sources.Constant const1(k=273.15 + 65)
      ...
      equation
        connect(const.y, prescribedTemperature.T) annotation (Line(
            ...
            color={0,0,127},
            smooth=Smooth.None));
        connect(prescribedTemperature.port, buoHeaTra1.TWat) annotation (Line(
            points={{-16,-6},{-6,-6},{-6,9.8},{41.4,9.8}},
            color={191,0,0},
            smooth=Smooth.None));
        connect(const1.y, prescribedTemperature1.T) annotation (Line(
            points={{-73,36},{-38,36}},
            color={0,0,127},
            smooth=Smooth.None));
        connect(prescribedTemperature1.port, buoHeaTra1.TWatBot) annotation (Line(
            points={{-16,36},{-4,36},{-4,20},{41.4,20}},
            color={191,0,0},
            smooth=Smooth.None));
        connect(realExpression1.y, buoHeaTra1.hBas) annotation (Line(
            points={{-69,74},{-8,74},{-8,22.2},{40,22.2}},
            color={0,0,127},
            smooth=Smooth.None));
        connect(realExpression1.y, buoHeaTra1.QDotBas) annotation (Line(
            points={{-69,74},{-8,74},{-8,17.6},{40,17.6}},
            color={0,0,127},
            smooth=Smooth.None));
        connect(realExpression3.y, buoHeaTra1.hWatFlu) annotation (Line(
            points={{-67,-46},{-12,-46},{-12,12},{40,12}},
            color={0,0,127},
            smooth=Smooth.None));
        connect(realExpression3.y, buoHeaTra1.QDotFlu) annotation (Line(
            points={{-67,-46},{-12,-46},{-12,7.4},{40,7.4}},
            color={0,0,127},
            smooth=Smooth.None));
        annotation (Diagram(graphics));
