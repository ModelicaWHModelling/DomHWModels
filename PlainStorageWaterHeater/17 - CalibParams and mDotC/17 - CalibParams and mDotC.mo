  ...
  package Examples
    ...
    model WaterHeater "Test model for water heater"
      ...
      Modelica.Blocks.Tables.CombiTable1Ds combiTable1Ds[nSeg](
        tableOnFile=true,
        tableName="Flow",
        fileName="C:/Program Files (x86)/Dymola 2012 FD01/Models/WaterHeater/current/modeling/branches/pgrant/PlainStorageWaterHeater/StorageTankDrawProfile.txt")
    ...
    equation
      ...
      connect(TRadMea.y, tan.TRadMea)  annotation (Line(
          ...
          color={0,0,127},
          smooth=Smooth.None));
      connect(tan.heaPor[6], T_sensor.port) annotation (Line(
          points={{28.3377,84.6059},{46,84.6059},{46,-11},{62,-11}},
          color={191,0,0},
          smooth=Smooth.None));
...
First implementation.
    ...
    model buoHeaTra
      ...
      parameter Real m_flow_sr = 0.01 * 0.063;
      parameter Real c_sr = 2;
      parameter Real m_flow_C = m_flow_sr * c_sr;
      ...
      mDotC mDotC1(m_flow_sr=m_flow_sr, c_sr=c_sr,
        nSeg=nSeg) annotation (Placement(transformation(extent={{26,28},{46,48}})));
    equation
      ...
      connect(WatT.T, qBuo.TWat) annotation (Line(
          points={{-80,10},{-80,-12},{-4,-12},{-4,6},{32,6}},
          color={0,0,127},
          smooth=Smooth.None));
      connect(mDCal1.mDotD, qBuo.mDotD) annotation (Line(
          points={{-6,10},{32,10},{32,10}},
          color={0,0,127},
          smooth=Smooth.None));
      ...
      connect(QDotBas, qBuo.QBas) annotation (Line(
          points={{-120,36},{16,36},{16,14},{32,14}},
          color={0,0,127},
          smooth=Smooth.None));
      connect(QDotFlu, qBuo.QFlu) annotation (Line(
          ...
          color={0,0,127},
          smooth=Smooth.None));
      connect(WatT.T, mDotC1.TWat) annotation (Line(
          points={{-80,10},{-80,38},{24,38}},
          color={0,0,127},
          smooth=Smooth.None));
      connect(mDotC1.mDotC, qBuo.mDotC) annotation (Line(
          points={{47.6,38},{50,38},{50,24},{32,24},{32,18}},
          color={0,0,127},
          smooth=Smooth.None));
...
First implementation.
    ...
    model mDotC "Identifies the recirculation flow rate for each segment"
      extends Modelica.Blocks.Interfaces.BlockIcon;
      import Buildings;
      ...
      Modelica.Blocks.Interfaces.RealInput TWat[nSeg]
        annotation (Placement(transformation(extent={{-140,-20},{-100,20}})));
      Modelica.Blocks.Interfaces.RealOutput mDotC[nSeg]
        annotation (Placement(transformation(extent={{100,-16},{132,16}})));
    equation
      Location[nSeg] = 0;
      mDotC[nSeg] = m_flow_sr * c_sr;
      a[nSeg] = 0;
      b[nSeg] = 0;
      for i in 1:(nSeg-1) loop
        Location[nSeg-i] = Buildings.Utilities.Math.Functions.smoothHeaviside(TWat[nSeg-i] - TWat[nSeg-i+1] - 1, smoDel) + Buildings.Utilities.Math.Functions.smoothHeaviside(Location[nSeg-i+1]-0.1,smoDel);

        mDotC[nSeg-i] = m_flow_sr * (Buildings.Utilities.Math.Functions.smoothHeaviside(Location[nSeg-i]-0.1, smoDel) + c_sr * Buildings.Utilities.Math.Functions.smoothHeaviside(0.9-Location[nSeg-i],smoDel));
        a[nSeg-i] =  Buildings.Utilities.Math.Functions.smoothHeaviside(Location[nSeg-i]-0.1, smoDel);
        b[nSeg-i] = Buildings.Utilities.Math.Functions.smoothHeaviside(0.9-Location[nSeg-i],smoDel);
      end for;
