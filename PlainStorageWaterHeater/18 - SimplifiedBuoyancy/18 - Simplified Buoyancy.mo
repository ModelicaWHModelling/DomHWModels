...
package WaterHeaterLib
  ...
  //       1].T;
    ...
    connect(heatFlowSensor1.Q_flow, buoHeaTra.QDotFlu) annotation (Line(
        points={{-93,32},{-88,32},{-88,163.6},{76.12,163.6}},
        color={0,0,127},
        smooth=Smooth.None));
    connect(buoHeaTra.QDotBas, heatFlowSensor.Q_flow) annotation (Line(
        points={{61.72,163.6},{61.72,166},{20,166},{20,-136},{-10,-136},{-10,
            -152}},
        color={0,0,127},
        smooth=Smooth.None));
    connect(buoHeaTra.TWat, vol.heatPort) annotation (Line(
        points={{86.2,161.08},{-34,161.08},{-34,62},{-12,62},{-12,20},{82,20}},
        color={191,0,0},
        smooth=Smooth.None));
                                                                annotation (
...
First implementation.
  ...
  package Examples
    ...
    model WaterHeater "Test model for water heater"
      ...
      Modelica.Blocks.Sources.Constant QBurner(k=1318)  annotation (Placement(
...
First implementation.
    ...
    model buoHeaTra
      ...
      WaterHeaterLib.BaseClasses.strLay
            strLay(nSeg=nSeg) annotation (Placement(transformation(extent={{-66,80},{-46,100}})));
...
First implementation.
    ...
    equation
      ...
      for i in 1:(nSeg-1) loop
        ...
        strLay[nSeg-i] = min(1,Buildings.Utilities.Math.Functions.smoothHeaviside(TWat[nSeg-i] - TWat[nSeg-i+1] - 0.5 - smoDel, smoDel) + Buildings.Utilities.Math.Functions.smoothHeaviside(strLay[nSeg-i+1]-0.1,smoDel));
    ...
    equation
      ...
      for i in 1:(nSeg-1) loop
        QIn[i] = heaPerSeg * Buildings.Utilities.Math.Functions.smoothHeaviside(0.5 - (TWat[i] - TWat[i+1]) - smoDel, smoDel) + QDotFlu[i] * Buildings.Utilities.Math.Functions.smoothHeaviside(TWat[i] - TWat[i+1] - 0.5 - smoDel, smoDel);
      end for;

      a = Buildings.Utilities.Math.Functions.smoothHeaviside(0.5 - (TWat[1] - TWat[2]) - smoDel, smoDel);
      b = Buildings.Utilities.Math.Functions.smoothHeaviside(TWat[1] - TWat[2] - 0.5 - smoDel, smoDel);
