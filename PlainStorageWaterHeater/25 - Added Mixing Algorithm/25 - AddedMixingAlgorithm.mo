...
First implementation.
    ...
    for i in (1:nSeg-1) loop
        ...
        draHeaTra[i] = perFloIn1.perFlo[i] * mFloIn[i] * c_p * (TIn - TWat[i]) + SumBelow[i] * mFloIn[i] * c_p * (TWat[i+1] - TWat[i]);
    end for;
...
First implementation.
    ...
    equation
      ...
      for i in 1:(nSeg-1) loop
        QIn[i] = heaPerSeg * Buildings.Utilities.Math.Functions.smoothHeaviside(0.1 - (TWat[i] - TWat[i+1]) - smoDel, smoDel) + QDotFlu[i] * Buildings.Utilities.Math.Functions.smoothHeaviside(TWat[i] - TWat[i+1] - 0.1 - smoDel, smoDel);
      end for;
