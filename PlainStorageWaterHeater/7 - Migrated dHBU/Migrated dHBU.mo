...
package WaterHeaterLib
  ...
  //     m_flow_out[nSeg];
    ...
    for i in 2:(nSeg - 1) loop
      Q_flow_tot[i] := buoHeaTra.y[i] + c_p*buoHeaTra.mDotD[i - 1]*(vol[i - 1].heatPort.T
    ...
    Q_flow_tot[1] := buoHeaTra.y[1] + c_p*m_flow_C[2]*(vol[2].heatPort.T - vol[1].heatPort.T);
    ...
    Q_flow_tot[nSeg] := buoHeaTra.y[nSeg] - c_p*m_flow_C[nSeg]*(vol[nSeg].heatPort.T
...
First implementation.
    ...
    model QBuo "Calculates the heat transfer rate caused by buoyancy"
      ...
      Modelica.Blocks.Interfaces.RealInput TWat[nSeg]
        annotation (Placement(transformation(extent={{-140,-40},{-100,0}})));
      Modelica.Blocks.Interfaces.RealInput mFloFlu[nSeg]
        annotation (Placement(transformation(extent={{-140,-80},{-100,-40}})));
    ...
    algorithm
      dH_BU_B[1] :=mDotBas*c_p*(THotBas - TWat[1])*
        Buildings.Utilities.Math.Functions.smoothHeaviside(THotBas - TWat[1],
        smoDel);

        for j in 1:nSeg loop
            dH_BU_F_Seg[j] :=mDotFlu[j]*c_p*(THotFlu[j] - TWat[1])*
          Buildings.Utilities.Math.Functions.smoothHeaviside(THotFlu[j] - TWat[
          1], smoDel);
        end for;
      ...
      for i in 2:(nSeg - 1) loop
          dH_BU_B[i] :=mDotBas*c_p*(THotBas - TWat[i])*
          Buildings.Utilities.Math.Functions.smoothHeaviside(THotBas - TWat[i],
          smoDel)*Buildings.Utilities.Math.Functions.smoothHeaviside(TWat[i - 1]
           - THotBas, smoDel);

        for j in 1:i loop
            dH_BU_F_Seg[j] :=mDotFlu[j]*c_p*(THotFlu[j] - TWat[i])*
            Buildings.Utilities.Math.Functions.smoothHeaviside(THotFlu[j] -
            TWat[i], smoDel)*Buildings.Utilities.Math.Functions.smoothHeaviside(
            TWat[i - 1] - THotFlu[j], smoDel);
        end for;
        dH_BU_F[i] :=sum(dH_BU_F_Seg[1:nSeg]);
        dH_BU[i] :=dH_BU_B[i] + dH_BU_F[i];
      end for;

      // heat enthalpy of zone nSeg, see Eqn 11. page 8 of water heater manual
        dH_BU_B[nSeg] :=mDotBas*c_p*(THotBas - TWat[nSeg])*
        Buildings.Utilities.Math.Functions.smoothHeaviside(TWat[nSeg - 1] -
        THotBas, smoDel);

      for j in 1:nSeg loop
          dH_BU_F_Seg[j] :=mDotFlu[j]*c_p*(THotFlu[j] - TWat[nSeg])*
          Buildings.Utilities.Math.Functions.smoothHeaviside(TWat[nSeg - 1] -
          THotFlu[j], smoDel);
      end for;
