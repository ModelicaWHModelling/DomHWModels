...
package WaterHeaterLib
  ...
  //   Q_flow_W := heatFlowSensor1.Q_flow;
  ...
  //         m_flow_F[j] := m_flow_F[j];
  //       else
  //         m_flow_F[j] := 0.0;
  //       end if;
  //     end for;
  //
  //     m_flow_F[i] := sum(m_flow_F[1:i]);
  //     m_flow_BU[i] := m_flow_BU[i] + m_flow_F[i];
  //   end for;
  //
  ...
  //   end for;
  //
  //   m_flow_F[nSeg] := sum(m_flow_F[1:nSeg]);
  //   m_flow_BU[nSeg] := m_flow_BU[nSeg] + m_flow_F[nSeg];
  ...
  //    end if;
    ...
    for i in 2:(nSeg - 1) loop
      Q_flow_tot[i] := deltaH_flow_BU[i] + c_p*buoHeaTra.mDotD[i - 1]*(vol[i - 1].heatPort.T
    ...
    Q_flow_tot[1] := deltaH_flow_BU[1] + c_p*m_flow_C[2]*(vol[2].heatPort.T - vol[1].heatPort.T);
    ...
    Q_flow_tot[nSeg] := deltaH_flow_BU[nSeg] - c_p*m_flow_C[nSeg]*(vol[nSeg].heatPort.T
       - vol[nSeg - 1].heatPort.T) + buoHeaTra.mDotD[nSeg-1] * c_p * (vol[nSeg-1].heatPort.T - vol[nSeg].heatPort.T);
  ...
  package Examples
    ...
    equation
      ...
      connect(tan.heaPor[2], T_sensor.port) annotation (Line(
          points={{28.3377,83.2537},{46,83.2537},{46,-11},{62,-11}},
          color={191,0,0},
          smooth=Smooth.None));
...
First implementation.
    ...
    equation
      ...
      connect(buoFlu.m_flow_buo, mDCal1.mDotFlu) annotation (Line(
          points={{-41,-36},{28,-36},{28,1},{38,1}},
          color={0,0,127},
          smooth=Smooth.None));
      connect(buoBas.m_flow_buo, mDCal1.mDotBas) annotation (Line(
          points={{-37,66},{22,66},{22,19},{38,19}},
          color={0,0,127},
          smooth=Smooth.None));
      connect(mDCal1.mDotD, mDotD) annotation (Line(
          points={{62,10},{110,10}},
          color={0,0,127},
          smooth=Smooth.None));
      annotation (Diagram(graphics));
