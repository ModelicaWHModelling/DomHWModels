...
First implementation.
    ...
    algorithm
        //  total buoyancy flow from lower zones which mixes into zone i, see Eqn 7. page 8 of water heater manual
      for i in 2:(nSeg-1) loop
        if ((TWat[i] < THotOutBas) and (THotOutBas <= TWat[i - 1])) then
          m_flow_BU[i] :=  mFloOutBas;
        else
          m_flow_BU[i] :=  0;
        end if;

         for j in 1:nSeg loop
           if ((TWat[i] < THotOutFlu[j]) and (THotOutFlu[j] <= TWat[i - 1])) then
             mFloInFluSeg[j] :=  mFloOutFlu[j];
           else
             mFloInFluSeg[j] :=  0;
           end if;
         end for;
      ...
      //  total buoyancy flow from lower zones which mixes into zone nSeg, see Eqn 7. page 8 of water heater manual
       if (TWat[2] < THotOutBas) then
         m_flow_BU[1] :=mFloOutBas;
       else
         m_flow_BU[1] :=0;
       end if;

        for j in 1:nSeg loop
          if (TWat[1] < THotOutFlu[j]) then
            mFloInFluSeg[j] :=  mFloOutFlu[j];
          else
            mFloInFluSeg[j] :=  0;
          end if;
      ...
      if (TWat[nSeg-1] > THotOutBas) then
        m_flow_BU[nSeg] := mFloOutBas;
      else
         m_flow_BU[nSeg] := 0;
      end if;
